...
def cleanScooter():
    df = pd.read_csv('D:\\PYTHON DATA ENGINEER\\src\\dataset\\scooter.csv')  # Update path
    df.drop(columns=['region_id'], inplace=True)
    df.columns = [x.lower() for x in df.columns]
    df['started_at'] = pd.to_datetime(df['started_at'], format='%m/%d/%Y %H:%M')
    df.to_csv('D:\\PYTHON DATA ENGINEER\\src\\dataset\\cleanscooter.csv', index=False)  # Update path

def filterData():
    df = pd.read_csv('D:\\PYTHON DATA ENGINEER\\src\\dataset\\cleanscooter.csv')  # Update path
    fromd = '2019-05-23'
    tod = '2019-06-03'
    tofrom = df[(df['started_at'] > fromd) & (df['started_at'] < tod)]
    tofrom.to_csv('D:\\PYTHON DATA ENGINEER\\src\\dataset\\may23-june3.csv', index=False)  # Update path
...
with DAG(
    ...
    moveFile = BashOperator(
        task_id='move',
        bash_command='mv /home/paulcrickard/may23-june3.csv /home/paulcrickard/Desktop'  # Update this path based on your OS
    )

# Task Dependencies
downloadData >> cleanData >> selectData >> moveFile
